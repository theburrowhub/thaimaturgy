...
func New(store *storage.Storage, config *domain.Config, provider providers.Provider) *Service {
	s := &Service{
		...
		autosaveCh: make(chan *domain.SessionState, 128),
	}
	go s.autosaveLoop()
	return s
}
...
// nameTaken reports whether a session name is already used — either persisted on
// disk or open in memory (a freshly created session isn't saved yet, so the
// registry must be consulted too).
func (s *Service) nameTaken(name string) bool {
	if s.store.SessionExists(name) {
		return true
	}
	s.mu.Lock()
	defer s.mu.Unlock()
...
// derived from the adventure id), registers it live, and returns its name.
...
// register binds a state+adventure into a live session (oracle, command handler,
// journal + log hook) and stores it in the registry.
func (s *Service) register(state *domain.SessionState, adv *domain.Adventure) (*OpenSession, error) {
	...
	journal, _ := s.store.OpenSessionJournal(state.Name)
	if journal != nil {
...
// SaveSession persists an open session synchronously.
func (s *Service) SaveSession(name string) error {
	os, ok := s.Get(name)
	if !ok {
		return fmt.Errorf("session %q is not open", name)
	}
	return s.store.SaveSession(os.Session.State)
}

// CloseSession persists, closes the journal, and unregisters a session.
...
// Autosave enqueues an open session for a background save (FIFO, so saves commit
// in request order). No-op if the session isn't open.
func (s *Service) Autosave(name string) {
	os, ok := s.Get(name)
	if !ok {
		return
	}
	s.autosaveCh <- os.Session.State
}

func (s *Service) autosaveLoop() {
	for state := range s.autosaveCh {
		_ = s.store.SaveSession(state)
		// Write progression back to roster-linked party members (#33); best effort.
		_, _ = s.store.SyncPartyToRoster(rosterLinked(state))
	}
}
